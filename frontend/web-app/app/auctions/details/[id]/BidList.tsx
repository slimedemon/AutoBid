        ...
        <div className="rounded-lg shadow-md">
            <div className="py-2 px-4 bg-white">
                <div className="sticky top-0 bg-white p-2">
                    <Heading title={`Current highest bid: $${numberWithCommas(highBid)}`} />
                </div>

            </div>

            <div className="overflow-auto h-80 flex flex-col-reverse px-2">
                {bids.length === 0 ? (
                    <EmptyFilter title="No bids for this item"
                        subtitle="Please feel free to make the first bid!" />
                ) : (
                    <>
                        {
                            bids.map(bid => (
